...
const Movie = ({item}) => {
    ...
    const saveShow = async () =>{
      if(user?.email){
        ...
        alert('Please login to save this movie!!')
      }
    };
  return (
    ...
    <div className='absolute top-0 left-0 w-full h-full hover:bg-black/80 opacity-0 hover:opacity-10 text-white'>
      ...
      <p onClick={saveShow}>{like ? <FaHeart className='absolute top-4 left-4 text-white-300 text-5xl' /> : <FaRegHeart className='absolute top-4 left-4 text-gray-300 text-2xl'/>}</p>
    </div>
  </div>
  )
}
